 ...
 * ================================================================
 * font_5x7.h
 * 5x7 pixel bitmap font — full printable ASCII (0x20 to 0x7E)
 * ================================================================
 * Format: column-major, 1 byte per column (7 bits used, LSB=top)
 * Each character: 5 columns x 1 byte = 5 bytes per glyph
 * Total glyphs: 95 (space through tilde)
 * Flash cost: 95 * 5 = 475 bytes
 *
 * Stored as const → lives in flash, zero RAM cost at runtime.
 *
 * HOW TO USE
 * ----------
 * #include "font_5x7.h"
 * ssd1306_draw_text(oled, 0, 0, "Hello World", &font_5x7, true);
 *
 * At scale 1 (natural): 5px wide + 1px gap = 6px per char
 *   → 21 characters fit per row on 128px display
 * ================================================================
 */
...
/* Column data for each character, LSB = top pixel of column */
static const uint8_t FONT_5X7_DATA[] = {
    ...
    /* 0x7E '~' */ 0x0C, 0x02, 0x0C, 0x10, 0x0C,
};

/**
 * Font descriptor — pass a pointer to this to ssd1306_draw_text().
 * Stored as const → zero RAM cost.
 */
static const ssd1306_font_t font_5x7 = {
    .width          = 5,
    .height         = 7,
    .char_spacing   = 1,
